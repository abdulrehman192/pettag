...
class PetSlideScreen extends StatefulWidget {
  ...
  PetSlideScreen({Key? key}) : super(key: key);
...
/*  void _loadInterstitialAd() {
  ...
  getPetType() async {
    ...
    if (snapshot.exists) {
      ...
      if (data.isNotEmpty) {
        ...
        _petType = data['interest'].toString();
  ...
  Future<Packages> getPackageName() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    print('pref :  ${prefs.containsKey("packageName")}');
  ...
  getPetId() async {
    await FirebaseFirestore.instance
      ...
      FirebaseFirestore.instance
        ...
        boostEndTimer = DateTime.now().millisecondsSinceEpoch + 20000 * 60;
      });
      isOffline();
    });
  }
  ...
  Future<dynamic> getUserLocation() async {
    ...
    if (!serviceEnabled) {
      ...
      if (!serviceEnabled) {
        return "Location Service not enable";
      }
    }
    ...
    if (permissionGranted == PermissionStatus.denied) {
      ...
      if (permissionGranted != PermissionStatus.granted) {
        print("permission not grant $permissionGranted");
    ...
    try {
      ...
      print("the location data = $locationData");
  ...
  getPreviousData() async {
    ...
    if (!prefs.containsKey('minAge') && !prefs.containsKey('maxAge')) {
      prefs.setDouble('minAge', 1);
      prefs.setDouble('maxAge', 5);
    }
    if (!prefs.containsKey('ownerMinAge') &&
        !prefs.containsKey('ownerMaxAge')) {
      prefs.setDouble('ownerMinAge', 18);
      prefs.setDouble('ownerMaxAge', 30);
    }
    if (!prefs.containsKey('radius')) {
      prefs.setInt('radius', 50);
    }
    if (!prefs.containsKey('preferredGender')) {
      prefs.setString('preferredGender', 'Both');
    }
    if (!prefs.containsKey('preferredOwnerGender')) {
      prefs.setString('preferredOwnerGender', 'Both');
    }
  }

  List<DocumentSnapshot<Object?>> docList = [];

  Future<List<DocumentSnapshot<Object?>>>? getDocs() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    final center = await getUserLocation();
    CollectionReference<Object?> query = FirebaseFirestore.instance.collection("Pet");
    GeoFirestore geoFirestore = GeoFirestore(query);
    print("the center is $center");
    print("the radius ${prefs.getInt('radius')!.toDouble()}");
    if (center.runtimeType == String) {
      print("${center.runtimeType} we got in error center runtime!!!");
      return Future.error(center);
    }
    return geoFirestore
        .getAtLocation(GeoPoint(center.latitude, center.longitude),
            prefs.getInt('radius')!.toDouble())
        .then((value) {
      if (kDebugMode) {
        for (var data in value) {
          print("the value in firestore ${data.id}");
        }
      }
      docList = [];
      docList.addAll(value);
      docList.sort((a, b) {
        Map<String, dynamic> val = b.data() as Map<String, dynamic>;
        return (val.containsKey('boosted') && val['boosted'] ? 1 : 0);
      });
      docList.sort((a, b) {
        var mapA = (a.data() as Map<String, dynamic>);
        var mapB = (b.data() as Map<String, dynamic>);
        int timeA = mapA.containsKey('boostedTimestamp')
            ? (a.data() as Map<String, dynamic>)['boostedTimestamp']
            : 0;
        int timeB = mapB.containsKey('boostedTimestamp')
            ? (b.data() as Map<String, dynamic>)['boostedTimestamp']
            : 0;
        return timeA.compareTo(timeB);
      });
      print("after the sort ${docList.length}");
      docList.removeWhere((element) =>
          (element.data() as Map<String, dynamic>)['ownerId'] ==
          auth.currentUser!.uid);
      print("after the remove ${docList.length}");
       var filterData = docList.where((element) {
        Map<String, dynamic> map = element.data() as Map<String, dynamic>;
        print(
            "pref gender ${map['sex']} ${prefs.getString('preferredGender')}");
        var age = map['age'] != null &&
            map['age'] >= prefs.getDouble('minAge') &&
            map['age'] <= prefs.getDouble('maxAge');
        var ownerAge = map['ownerAge'] != null &&
            map['ownerAge'] >= prefs.getDouble('ownerMinAge') &&
            map['ownerAge'] <= prefs.getDouble('ownerMaxAge');
        var gender = (prefs.getString('preferredGender') == 'Both')
            ? (map['sex'] == 'Male' || map['sex'] == 'Female')
            : map['sex'] == prefs.getString('preferredGender');
        var ownerGender = (prefs.getString('preferredOwnerGender') == 'Both')
            ? (map['sex'] == 'Male' || map['sex'] == 'Female')
            : map['ownerGender'] == prefs.getString('preferredOwnerGender');
        var filter = age && ownerAge && gender && ownerGender;
        print(
            'filterData ${map['ownerId']} $age $ownerAge $gender $ownerGender $filter');

        return filter;
      }) .toList();
      print("after ${filterData.length}");
      return filterData ;
    });
  }
                            ...
                            ? FutureBuilder(
                                ...
                                builder: (context,
                                  ...
                                  if (snapshot.hasError) {
                                    print(
                                        "the snapshot error ${snapshot.error}");
                                    return Center(
                                      child: Text(
                                        "${snapshot.error}",
                                      ...
                                      return TinderSwapCard(
                                        ...
                                        swipeCompleteCallback:
                                            (CardSwipeOrientation orientation,
                                                int index) async {
                                          print("$_index the index from class");
                                          _index += 1;
                                                                  ...
                                                                  Positioned(
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      children: [
                                                                        ...
                                                                        Text(
                                                                          "${(mp.SphericalUtil.computeDistanceBetween(mp.LatLng(lat, lng), mp.LatLng(data['latitude'], data['longitude'])) / 1000).round().toStringAsFixed(2)} Km Away",
                                                                          style:
                                                                              const TextStyle(
                                                                            fontSize:
                                                                                13,
                              ...
                              onPressed: () async {
                                ...
                                if (isBoosted) {
                                  boostedTimeController =
                                      CountdownTimerController(endTime: DateTime.now()
                                          .millisecondsSinceEpoch + 1000 * 1200);
                                  setState(() {});
                                }
                              },
                              height: 40,
