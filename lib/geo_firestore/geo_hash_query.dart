...
class GeoHashQuery {
  ...
  GeoHashQuery _joinWith(GeoHashQuery other) {
    if (other._isPrefix(this)) {
      return GeoHashQuery(
          startValue: startValue, endValue: other.endValue);
    }
    if (_isPrefix(other)) {
      return GeoHashQuery(
          startValue: other.startValue, endValue: endValue);
    }
    if (_isSuperQuery(other)) {
      return other;
    }
  ...
  bool containsGeoHash(String hash) {
    return startValue!.compareTo(hash) <= 0 &&
        endValue!.compareTo(hash) > 0;
  }
